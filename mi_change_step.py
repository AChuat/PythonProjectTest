...
def login(user, password):
    ...
    print(f"登录token请求：{url1}\n {json.dumps(data1, indent=4)}")
    r1 = requests.post(url1, data=data1, headers=headers, allow_redirects=False)
    print(f"登录token返回headers：\n {json.dumps(dict(r1.headers), indent=4)}")
    print(f"登录token返回body：\n {r1.text}")
    location = r1.headers["Location"]
    try:
        code = get_code(location)
        print(f"登录token返回code：{code}\n")
    except:
        return 0, 0
...
# 主函数
def main_handle(user, passwd, step, sckey):
    user = str(user)
    password = str(passwd)
    step = str(step)
    if user == '' or password == '':
...
step = str(8888 + random.randint(987, 8732))
setp_array = step.split('-')
if len(setp_array) == 2:
    step = str(random.randint(int(setp_array[0]), int(setp_array[1])))
elif str(step) == '0':
    step = ''
main_handle(user, passwd, step, sckey)
"""
